...
async function fetchText(url, options = {}) {
  ...
  try {
    const response = await fetch(url, {
      ...
      headers: {
        'User-Agent': 'ConsultaDiariaBOCM/0.16',
...
export async function searchBocm(date, municipalityText = '') {
  ...
  // Se leen varios anuncios en paralelo con un límite controlado. La versión
  // anterior procesaba tandas pequeñas y podía agotar el tiempo máximo de Vercel,
  // que entonces devolvía una página de error en texto en lugar de JSON.
  const readResults = await mapWithConcurrency(links, 20, readAnnouncement);
